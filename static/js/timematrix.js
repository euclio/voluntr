(function() {
    ...
    /*
     * Create a table representing the list of times inside any element with
     * the class 'timematrix'.
     */
    ...
    function selectCells(selectedTimes) {
        ...
        for (var i = 0; i < selectedTimes.length; i++) {
            var selectedTime = moment(
                selectedTimes[i].startTime + selectedTimes[i].dayOfWeek,
                'HH:mm:ssdddd');
    ...
    function addListeners() {
         ...
         *
         * If the event is a click, we apply the class 'selected' to the
         * element. We also mark the cell as being already toggled, so if we
         * mouse over it again in the same drag it is not toggled again.
         */
        ...
        // Apply the listeners to all table cells except the time labels.
        // We must trigger both the mouse over and mousedown events to ensure
        // that the both the first cell and subsequent cells are triggered when
        // the mouse is dragged over them.
        $('table tbody td:not(:first-child)', $timeMatrix)
            .mousedown(selectCell)
            .mouseover(selectCell);

        // When the mouse is let up anywhere on the page, we mark all the cells
        // as not already toggled.
    ...
    $(document).ready(function() {
        ...
        // Add the times that are currently selected to the form right before
        // it is submitted.
